        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
            <div>
                <h2 class="text-title-md2 font-bold text-black dark:text-white">
                    Laporan Minggu Ke-<?php echo e($summary['minggu_ke']); ?> (Bulan <?php echo e($monthName); ?> <?php echo e($year); ?>)
                </h2>
                <p class="text-sm mt-1 text-slate-500 dark:text-slate-400">
                    <?php if($summary['nama_proyek'] === 'all'): ?>
            ...
            <div class="flex flex-wrap items-center justify-end gap-3 mt-4 sm:mt-0">
                ...
                <?php if($summary['nama_proyek'] !== 'all'): ?>
                    <button type="button" onclick="pilihJenisLaporanMingguan('<?php echo e(route('pdf.weekly', ['minggu' => $summary['minggu_ke'], 'proyek' => $summary['nama_proyek'], 'year' => $year, 'month' => $month])); ?>', '<?php echo e(route('excel.weekly', ['minggu' => $summary['minggu_ke'], 'proyek' => $summary['nama_proyek'], 'year' => $year, 'month' => $month])); ?>', '<?php echo e(route('pdf.weekly.physical', ['minggu' => $summary['minggu_ke'], 'proyek' => $summary['nama_proyek'], 'year' => $year, 'month' => $month])); ?>', '<?php echo e(route('excel.weekly.physical', ['minggu' => $summary['minggu_ke'], 'proyek' => $summary['nama_proyek'], 'year' => $year, 'month' => $month])); ?>')" class="inline-flex items-center justify-center rounded-md bg-blue-600 py-2.5 px-5 text-sm font-medium text-white hover:bg-blue-700 transition">Cetak Mingguan</button>
                <?php else: ?>
                    <button type="button" onclick="cetakDariSemuaProyek(<?php echo e($summary['minggu_ke']); ?>)" class="inline-flex items-center justify-center rounded-md bg-blue-600 py-2.5 px-5 text-sm font-medium text-white hover:bg-blue-700 transition">Cetak Mingguan</button>
                <?php endif; ?>
            </div>
        </div>
                ...
                <div>
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <div class="flex justify-between border-b border-stroke pb-3 dark:border-strokedark">
                            ...
                            <span
                                class="text-sm font-medium text-black dark:text-white text-right"><?php echo e($summary['tahun_anggaran']); ?></span>
                        </div>
            ...
            <div class="max-w-full overflow-x-auto pb-4">
                <table class="w-full table-auto">
                    ...
                    <tbody class="divide-y divide-stroke dark:divide-strokedark">
                        <?php $__empty_1 = true; $__currentLoopData = $rekapFisik; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <?php if(!empty($row['is_header'])): ?>
                                <tr class="bg-gray-100 dark:bg-meta-4/30">
                                    <td class="py-3 px-4 text-left font-bold text-black dark:text-white pl-4"><?php echo e($row['no']); ?></td>
                                    <td class="py-3 px-4 font-bold text-black dark:text-white" colspan="2"><?php echo e($row['item']); ?></td>
                                </tr>
                            <?php else: ?>
                                <tr class="hover:bg-gray-50 dark:hover:bg-meta-4/50 transition text-sm">
                                    <td class="py-3 px-4 text-right text-black dark:text-white pr-4"><?php echo e($row['no']); ?></td>
                                    <td class="py-3 px-4 text-gray-800 dark:text-gray-200"><?php echo e($row['item']); ?></td>
                                    <td class="py-3 px-4 text-center text-gray-800 dark:text-gray-200"><?php echo e(number_format($row['sampai_bobot'] * 100, 3)); ?>%</td>
                                </tr>
                            <?php endif; ?>
            ...
            <div class="max-w-full overflow-x-auto pb-4">
                <table class="w-full table-auto" id="tabelHarian">
                    ...
                    <tbody class="divide-y divide-stroke dark:divide-strokedark" id="tableBody">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-meta-4/50 transition">
                                ...
                                <td class="py-4 px-4 text-center align-middle">
                                    <div class="flex flex-col gap-1.5">
                                        <a href="<?php echo e(route('laporan.show', $laporan->id)); ?>" class="w-full inline-flex justify-center items-center rounded bg-blue-50 py-1.5 px-2 text-xs font-medium text-blue-600 hover:bg-blue-500 hover:text-white dark:bg-blue-500/10 dark:text-blue-400 transition">Detail</a>
                                    </div>
                                </td>
                                <?php endif; ?>
                            </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                            <tr>
                                <td colspan="<?php echo e($summary['nama_proyek'] === 'all' ? '6' : '5'); ?>" class="text-center py-10 text-gray-500 dark:text-gray-400">Belum ada laporan
...
<script>
    async function cetakDariSemuaProyek(minggu) {
        ...
        if (proyek) {
            const urlLamaPdf = `<?php echo e(url('pdf/weekly')); ?>/${minggu}/${encodeURIComponent(proyek)}?year=<?php echo e($year); ?>&month=<?php echo e($month); ?>`;
            const urlLamaExcel = `<?php echo e(url('excel/weekly')); ?>/${minggu}/${encodeURIComponent(proyek)}?year=<?php echo e($year); ?>&month=<?php echo e($month); ?>`;
            const urlBaruPdf = `<?php echo e(url('pdf/weekly-fisik')); ?>/${minggu}/${encodeURIComponent(proyek)}?year=<?php echo e($year); ?>&month=<?php echo e($month); ?>`;
            const urlBaruExcel = `<?php echo e(url('excel/weekly-fisik')); ?>/${minggu}/${encodeURIComponent(proyek)}?year=<?php echo e($year); ?>&month=<?php echo e($month); ?>`;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\PROJECT FANY\Bot-Monitoring-Pekerjaan\resources\views/weekly/show.blade.php ENDPATH**/ ?>