...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="rounded-xl bg-blue-600 px-6 py-4 text-white shadow-sm flex flex-col items-end shrink-0">
            ...
            <h3 class="text-xl font-bold">{{ now()->translatedFormat('d F Y') }}</h3>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        ...
        @endphp
        @foreach($cards as $c)
        <div class="rounded-xl border border-gray-200 bg-white py-6 px-7 shadow-sm dark:border-gray-800 dark:bg-gray-900 hover:shadow-md transition" data-testid="{{ $c['testid'] }}">
            <div class="flex h-11 w-11 items-center justify-center rounded-full {{ $c['iconBg'] }}">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="rounded-xl border border-gray-200 bg-white px-5 pt-7 pb-5 shadow-sm dark:border-gray-800 dark:bg-gray-900 sm:px-7.5 h-full flex flex-col">
            ...
            <div class="mt-6 space-y-3 text-sm">
                <div class="flex justify-between pb-2 border-b border-gray-100 dark:border-gray-800"><span class="text-gray-600 dark:text-gray-300">Disetujui</span><span class="font-bold text-green-600 dark:text-green-500">{{ $disetujui }}</span></div>
                <div class="flex justify-between pb-2 border-b border-gray-100 dark:border-gray-800"><span class="text-gray-600 dark:text-gray-300">Menunggu</span><span class="font-bold text-yellow-600 dark:text-yellow-500">{{ $menunggu }}</span></div>
                <div class="flex justify-between"><span class="text-gray-600 dark:text-gray-300">Ditolak</span><span class="font-bold text-red-600 dark:text-red-500">{{ $ditolak }}</span></div>
            </div>
        </div>
    </div>

    {{-- QUICK STATS & RECENT VERIFICATIONS --}}
            ...
            <div class="space-y-6">
                ...
                <div>
                    <div class="flex justify-between mb-2 text-sm">
                        ...
                        <span class="font-bold text-{{ $col }}-600 dark:text-{{ $col }}-400">{{ $val }}</span>
                    </div>
                    <div class="w-full h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                        <div class="h-full rounded-full bg-{{ $col }}-500 transition-all duration-500 ease-out" style="width: {{ $w }}%"></div>
                    </div>
                </div>
                @endforeach
            </div>
    ...
    <div class="rounded-xl border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-sm dark:border-strokedark dark:bg-boxdark sm:px-7.5" data-testid="recent-reports">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('laporan.index') }}" class="px-5 py-2 text-sm font-medium rounded-md bg-blue-600 hover:bg-blue-700 text-white transition flex-shrink-0">
                Lihat Semua
            </a>
        </div>
            ...
            <table class="w-full table-auto">
                ...
                <tbody class="divide-y divide-stroke dark:divide-strokedark">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-meta-4/50 transition">
                        <td class="py-4 px-2 text-sm font-medium text-black dark:text-white">{{ $laporan->tanggal->format('d M Y') }}</td>
                        <td class="py-4 px-2 text-sm font-semibold text-gray-800 dark:text-gray-200">{{ $laporan->nama_proyek }}</td>
                        <td class="py-4 px-2 text-sm font-medium text-gray-800 dark:text-gray-200">{{ $laporan->karyawan->nama ?? '-' }}</td>
                        ...
                        <td class="py-4 px-2 text-center">
                            <a href="{{ route('laporan.show',$laporan) }}" class="inline-flex rounded bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-600 hover:bg-blue-600 hover:text-white transition dark:bg-blue-500/20 dark:text-blue-400 dark:hover:bg-blue-500 dark:hover:text-white">Detail</a>
                        </td>
                    </tr>
                    @empty
...
@push('scripts')
<script>
const weekly  = { labels: @json($weeklyLabels),  data: @json($weeklyData) };
...
new Chart(document.getElementById('statusChart'),{
    type:'doughnut',
    data:{ labels:['Disetujui','Menunggu','Ditolak'], datasets:[{ data:@json($statusChart), backgroundColor:['#10b981','#f59e0b','#ef4444'], borderWidth:0, hoverOffset: 4 }]},
