...
class WeeklyPdfController extends Controller
{
    public function weekly($minggu, $proyek)
    {
        ...
        foreach ($proyekList as $p) {
            try {
                $projectsData[] = $this->buildProjectData($minggu, $p);
            } catch (\Exception $e) {
                // Skip if no data
            }
        }
        ...
        if ($proyek === 'all') {
            $pdf = Pdf::loadView('pdf.weekly-all', ['projects' => $projectsData])->setPaper('A4', 'portrait');
            return $pdf->stream('Laporan-Mingguan-Semua-Proyek-Minggu-' . $minggu . '.pdf');
        } else {
            $data = $projectsData[0];
            $pdf = Pdf::loadView('pdf.weekly', $data)->setPaper('A4', 'portrait');
            return $pdf->stream('Laporan-Mingguan-' . $data['summary']['nama_proyek'] . '-Minggu-' . $minggu . '.pdf');
        }
    }

    private function buildProjectData($minggu, $proyek)
    {
        $semuaLaporan = Laporan::with([
            'karyawan', 'pekerjaans', 'tenagas', 'materials', 'alats', 'fotos', 'verifikasi.user'
        ])->where('nama_proyek', $proyek)->orderBy('tanggal')->get();

        if ($semuaLaporan->isEmpty()) throw new \Exception('Kosong');

        $tanggalMulaiProyek = Carbon::parse($semuaLaporan->min('tanggal'));
        ...
        if ($minggu === 'custom' && request()->has('start') && request()->has('end')) {
            ...
            if (request()->has('year') && request()->has('month')) {
                ...
                $laporans = $semuaLaporan->filter(function ($laporan) use ($tanggalMulaiProyek, $minggu) {
                    return $this->resolveWeekKey($laporan, $tanggalMulaiProyek) === (string) $minggu;
                })->values();
            }
        }

        if ($laporans->isEmpty()) throw new \Exception('Kosong');
        ...
        foreach ($laporans as $laporan) {
            foreach ($laporan->materials as $material) {
                ...
                if (!isset($materials[$nama])) {
                    $materials[$nama] = ['nama_material' => $nama, 'volume' => 0, 'satuan' => $material->satuan];
                }
        ...
        foreach ($laporans as $laporan) {
            foreach ($laporan->alats as $alat) {
                ...
                if (!isset($alats[$nama])) {
                    $alats[$nama] = ['nama_alat' => $nama, 'jumlah' => 0];
                }
        ...
        $tenagas = ['Pekerja' => 0, 'Tukang' => 0, 'Mandor' => 0, 'Pelaksana' => 0];
        foreach ($laporans as $laporan) {
        ...
        foreach ($laporans as $laporan) {
            foreach ($laporan->pekerjaans as $pekerjaan) {
                ...
                if (!isset($pekerjaans[$nama])) {
                    $pekerjaans[$nama] = ['nama_pekerjaan' => $nama, 'jumlah' => 0];
                }
    ...
    private function resolveWeekKey(Laporan $laporan, Carbon $tanggalMulaiProyek): string
    {
        ...
        if ($mingguKe !== '') return $mingguKe;
        $selisihHari = $tanggalMulaiProyek->diffInDays(Carbon::parse($laporan->tanggal));
