    ...
    public function weekly(string $minggu, string $proyek)
    {
        ...
        foreach ($proyekList as $p) {
            $laporans = Laporan::with(['karyawan', 'pekerjaans', 'materials', 'alats', 'tenagas', 'fotos'])
                ->where('nama_proyek', $p)->orderBy('tanggal')->get();
                
            if ($laporans->isEmpty()) continue;
        ...
        $filename = 'Laporan-Mingguan-Lama-' . ($proyek === 'all' ? 'Semua-Proyek' : Str::slug($proyek)) . '-Minggu-' . $minggu . '.xlsx';
